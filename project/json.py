...
def restaurantMenuJSON(restaurant_id):
    ...
    items = db.session.execute(text(query), {"restaurant_id": restaurant_id})
...
def menuItemJSON(restaurant_id, menu_id):
    query = "SELECT * FROM menu_item WHERE id = %s LIMIT 1"
    menu_item = db.session.execute(text(query), {"menu_id", menu_id,})
