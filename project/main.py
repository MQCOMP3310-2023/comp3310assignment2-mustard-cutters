...
def newRestaurant(owner_id):
    ...
    if owner.role == "admin" or owner.role == "restaurant_owner":
...
def editRestaurant(restaurant_id, owner_id):
    ...
    owner = db.session.query(User).filter_by(id=owner_id).first()
    if owner.role == "admin" or editedRestaurant.owner_id == owner_id:
        if request.method == 'POST':
...
def deleteRestaurant(restaurant_id, owner_id):
    ...
    owner = db.session.query(User).filter_by(id=owner_id).first()
    if owner.role == "admin" or restaurantToDelete.owner_id == owner_id:
        if request.method == 'POST':
...
def newMenuItem(restaurant_id, owner_id):
    ...
    global error 
    owner = db.session.query(User).filter_by(id=owner_id).first()
    if owner.role == "admin" or restaurant.owner_id == owner_id:
        if request.method == 'POST':
            error = False
